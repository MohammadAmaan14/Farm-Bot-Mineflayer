...
function handleChatMessage(bot, username, message) {
    ...
    // Check if the message from the Minecraft server instructs to use "/register" command
    if (message === '/register <password> <ConfirmPassword>') {
        const registerCommand = '/register bot112233 bot112233';
        bot.chat(registerCommand);
        console.log(`Sent command: ${registerCommand}`);
    }

    // Add other command handling logic as needed

    if (message === '/login <password>') {
        const loginCommand = '/login bot112233';
        bot.chat(loginCommand);
        console.log(`Sent command: ${loginCommand}`);
    }
}
...
const initBot = () => {
    ...
    bot.on('message', (message) => {
      console.log(nowStr(), 'Received message:', message.toAnsi());
    });
    
    bot.on('chat', (username, message) => {
        handleChatMessage(bot, username, message); // Pass 'bot' as the first parameter
    });
